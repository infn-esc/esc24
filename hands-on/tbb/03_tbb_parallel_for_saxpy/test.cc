...
template <typename T>
void serial_axpy(T a, std::vector<T> const& x, std::vector<T> const& y, std::vector<T>& z) {
...
template <typename T>
void measure_serial(T a, std::vector<T> const& x, std::vector<T> const& y) {
...
int main() {
  ...
  std::cout << "serial_axpy\n";
  for (size_t i = 0; i < times; ++i)
    measure_serial(a, x, y);
  std::cout << '\n';

  std::cout << "parallel_axpy\n";
  for (size_t i = 0; i < times; ++i)
